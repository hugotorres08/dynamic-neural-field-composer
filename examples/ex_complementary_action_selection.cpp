...
int main()
{
	try
	{
		...
		const auto simulation = std::make_shared<Simulation>("example complementary action selection", 5.0, 0.0, 0.0);
		...
		// external stimuli
		const auto hpscp = element::ElementCommonParameters{ "hand position stimulus" };
		...
		// fields
		const auto hpfcp = element::ElementCommonParameters{ "hand position field" };
		...
		hpf->addInput(hps);
		const auto sofcp = element::ElementCommonParameters{ "small object field" };
		...
		sof->addInput(so1s);
		sof->addInput(so2s);
		const auto lofcp = element::ElementCommonParameters{ "large object field" };
		...
		lof->addInput(los);
		const auto aefcp = element::ElementCommonParameters{ "action execution field" };
		...
		const auto sosfcp = element::ElementCommonParameters{ "small object selection field" };
		...
		const auto loifcp = element::ElementCommonParameters{ "large object integration field" };
		...
		const auto hpfkcp = element::ElementCommonParameters{ "hpf kernel" };
		...
		hpf->addInput(hpfk);
		hpfk->addInput(hpf);
		const auto sofkcp = element::ElementCommonParameters{ "sof kernel" };
		...
		sof->addInput(sofk);
		sofk->addInput(sof);
		const auto lofkcp = element::ElementCommonParameters{ "lof kernel" };
		...
		lof->addInput(lofk);
		lofk->addInput(lof);
		const auto sosfkcp = element::ElementCommonParameters{ "sosf kernel" };
		...
		sos->addInput(sosfk);
		sosfk->addInput(sos);
		const auto loifkcp = element::ElementCommonParameters{ "loif kernel" };
		...
		loif->addInput(loifk);
		loifk->addInput(loif);
		const auto aefkcp = element::ElementCommonParameters{ "aef kernel" };
		...
		// couplings
		const auto hpf_loif_cp = element::ElementCommonParameters{ "hpf - loif coupling" };
		...
		const auto hpf_sosf_cp = element::ElementCommonParameters{ "hpf - sosf coupling" };
		...
		const auto sof_sosf_cp = element::ElementCommonParameters{ "sof - sosf coupling" };
		...
		const auto lof_loif_cp = element::ElementCommonParameters{ "lof - loif coupling" };
		...
		const auto sosf_aef_cp = element::ElementCommonParameters{ "sosf - aef coupling" };
		...
		const auto loif_aef_cp = element::ElementCommonParameters{ "loif - aef coupling" };
		...
		aef->addInput(loif_aef);

		// normal noise
		const auto nnsosfcp = element::ElementCommonParameters{ "sosf normal noise" };
		...
		const auto nnaefcp = element::ElementCommonParameters{ "aef normal noise" };
