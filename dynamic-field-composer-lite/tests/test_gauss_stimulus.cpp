...
TEST_CASE("GaussStimulus class tests", "[GaussStimulus]")
{
    ...
    SECTION("init() method")
    {
        ...
        REQUIRE(gaussStimulus.getComponent("output")[0] == Catch::Approx(amplitude * dnf_composer::mathtools::circularGauss(size, sigma, position)[0]));
        REQUIRE(gaussStimulus.getComponent("output")[1] == Catch::Approx(amplitude * dnf_composer::mathtools::circularGauss(size, sigma, position)[1]));
        REQUIRE(gaussStimulus.getComponent("output")[2] == Catch::Approx(amplitude * dnf_composer::mathtools::circularGauss(size, sigma, position)[2]));
    }
