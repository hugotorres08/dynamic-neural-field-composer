...
namespace dnf_composer
{
	namespace user_interface
	{
		...
		void PlotControlWindow::renderPlotWindows() const
		{
			...
			//std::unordered_map<int, bool> openStates; // Tracks open/close state for each plot window
			...
			for (const auto& plot : plots)
			{
				...
				// If the window was closed by the user, remove it from openStates
				if (!open) {
		...
		void PlotControlWindow::renderLinePlot(const Plot& plot) const
		{
