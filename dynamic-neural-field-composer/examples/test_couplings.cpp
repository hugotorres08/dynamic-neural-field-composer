...
int main()
{
	try
	{
		...
		const element::ElementDimensions input_dimensions{360, 1.0};
		const auto nf_1 = factory.createElement(element::ElementLabel::NEURAL_FIELD,
			element::ElementCommonParameters{element::ElementIdentifiers{"input field" }, input_dimensions},
			element::NeuralFieldParameters{});
		const auto gk_1 = factory.createElement(element::GAUSS_KERNEL,
			element::ElementCommonParameters{element::ElementIdentifiers{"kernel input field"}, input_dimensions},
			element::GaussKernelParameters{});
		const auto nn_1 = factory.createElement(element::NORMAL_NOISE,
			element::ElementCommonParameters{element::ElementIdentifiers{"normal noise input field"}, input_dimensions},
			element::NormalNoiseParameters{});
		const auto gs_1 = factory.createElement(element::GAUSS_STIMULUS,
			element::ElementCommonParameters{element::ElementIdentifiers{"stimulus input field"}, input_dimensions},
		...
		const auto fc_1 = factory.createElement(element::FIELD_COUPLING,
			element::ElementCommonParameters{element::ElementIdentifiers{"coupling input-output"}, output_dimensions},
		...
		visualization->plot(
			...
			PlotType::LINE_PLOT,
			PlotDimensions{ 0.0, 360, -20.0, 20, 1.0, 1.0},
		...
		visualization->plot(
			PlotCommonParameters{
				PlotType::HEATMAP, 
				PlotDimensions{0.0, 100, 0.0, 360, 1.0, 1.0}, 
